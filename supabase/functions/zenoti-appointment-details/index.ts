...
Deno.serve(async (req) => {
  ...
  if (!appointment_id) {
    return new Response(JSON.stringify({ success: false, error: 'Missing appointment_id' }), { status: 400 });
  }

  try {
    const response = await fetch(`${zenotiApiUrl}/appointments/${appointment_id}`, {
