...
async function initialize(app, imageMatcher) {
  try {
    ...
    try {
      ...
      // Explicitly pass Supabase if it's not already set
      if (themeManager && !themeManager.supabase && global.supabaseClient) {
        themeManager.supabase = global.supabaseClient;
        logger.info("Explicitly passed Supabase client to ThemeManager");
      }
