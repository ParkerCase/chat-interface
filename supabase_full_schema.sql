...
CREATE OR REPLACE FUNCTION "public"."check_admin_status"() RETURNS TABLE("is_admin" boolean, "roles" "text"[])
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
  SELECT 
    (roles::text[] && ARRAY['admin'::text, 'super_admin'::text]) as is_admin,
    roles
  FROM profiles
  WHERE id = auth.uid();
END;
...
CREATE OR REPLACE FUNCTION "public"."get_all_profiles"() RETURNS SETOF "public"."profiles"
...
ALTER FUNCTION "public"."get_all_profiles"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_analytics_data"() RETURNS TABLE("total_users" bigint, "active_users" bigint, "total_messages" bigint, "files_processed" bigint)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
...
BEGIN
  ...
  SELECT 
    ...
    (SELECT COUNT(*) FROM profiles WHERE last_active > thirty_days_ago) as active_users,
    (SELECT COUNT(*) FROM chat_history) as total_messages,
    (SELECT COUNT(*) FROM file_processing_log WHERE status = 'completed') as files_processed;
END;
$$;


ALTER FUNCTION "public"."get_analytics_data"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_storage_stats"() RETURNS TABLE("bucket_name" "text", "file_count" integer, "total_size" bigint, "created_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  IF NOT EXISTS (
    ...
    ('admin' = ANY (roles) OR 'super_admin' = ANY (roles))
  ) THEN
    RAISE EXCEPTION 'Access denied. Admin privileges required.';
  END IF;
...
CREATE OR REPLACE FUNCTION "public"."slack_get_channels"() RETURNS "json"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    result JSON;
...
BEGIN
    ...
    FROM (
        ...
        OR (
            user_roles IS NOT NULL AND 
            ('admin' = ANY(user_roles) OR 'super_admin' = ANY(user_roles))
        )
        ORDER BY name
    ) c;
...
CREATE OR REPLACE FUNCTION "public"."validate_tatt2away_email"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  -- Check if email ends with @tatt2away.com
  IF RIGHT(NEW.email, 14) != '@tatt2away.com' THEN
    -- If not, delete the user and raise exception
...
CREATE FOREIGN DATA WRAPPER "tatt2awai" HANDLER "extensions"."wasm_fdw_handler" VALIDATOR "extensions"."wasm_fdw_validator";




CREATE SERVER "tatt2awai_server" FOREIGN DATA WRAPPER "tatt2awai" OPTIONS (
    "api_token_id" '0584961d-3742-4442-b7cf-e10a80bfe6c8',
...
ALTER SERVER "tatt2awai_server" OWNER TO "postgres";
...
CREATE POLICY "API keys are only manageable by admins" ON "public"."api_keys" TO "authenticated" USING (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
...
CREATE POLICY "Admin full access to profiles" ON "public"."profiles" USING ("public"."is_admin_safe"());



CREATE POLICY "Admins can create channels" ON "public"."slack_channels" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles")))))));



CREATE POLICY "Admins can create profiles" ON "public"."profiles" FOR INSERT WITH CHECK ("public"."is_admin"());



CREATE POLICY "Admins can do everything" ON "public"."profiles" USING ((EXISTS ( SELECT 1
   FROM "auth"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND (("users"."email")::"text" = ANY ((ARRAY['itsus@tatt2away.com'::character varying, 'parker@tatt2away.com'::character varying])::"text"[]))))));



CREATE POLICY "Admins can insert integrations" ON "public"."integrations" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."roles" && ARRAY['admin'::"text", 'super_admin'::"text"])))));



CREATE POLICY "Admins can insert settings" ON "public"."settings" FOR INSERT WITH CHECK ("public"."is_admin_safe"());



CREATE POLICY "Admins can manage all sessions" ON "public"."sessions" TO "authenticated" USING (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles")))))) WITH CHECK (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles"))))));
...
CREATE POLICY "Admins can manage sync queue" ON "public"."crm_sync_queue" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."roles" && ARRAY['admin'::"text", 'super_admin'::"text"])))));



CREATE POLICY "Admins can see all profiles" ON "public"."profiles" FOR SELECT USING ("public"."is_admin_safe"());



CREATE POLICY "Admins can update all profiles" ON "public"."profiles" FOR UPDATE USING ("public"."is_admin_safe"()) WITH CHECK ("public"."is_admin_safe"());



CREATE POLICY "Admins can update analytics stats" ON "public"."analytics_stats" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."roles" && ARRAY['admin'::"text", 'super_admin'::"text"])))));



CREATE POLICY "Admins can update integrations" ON "public"."integrations" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."roles" && ARRAY['admin'::"text", 'super_admin'::"text"])))));



CREATE POLICY "Admins can update settings" ON "public"."settings" FOR UPDATE USING ("public"."is_admin_safe"()) WITH CHECK ("public"."is_admin_safe"());



CREATE POLICY "Admins can update themes" ON "public"."themes" FOR UPDATE USING ("public"."is_admin"()) WITH CHECK ("public"."is_admin"());



CREATE POLICY "Admins can view all file processing logs" ON "public"."file_processing_log" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles")))))));



CREATE POLICY "Admins can view all integrations" ON "public"."integrations" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."roles" && ARRAY['admin'::"text", 'super_admin'::"text"])))));



CREATE POLICY "Admins can view all sessions" ON "public"."sessions" FOR SELECT TO "authenticated" USING (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles"))))));



CREATE POLICY "Admins can view analytics stats" ON "public"."analytics_stats" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."roles" && ARRAY['admin'::"text", 'super_admin'::"text"])))));



CREATE POLICY "Admins can view and manage all dashboard presets" ON "public"."dashboard_presets" TO "authenticated" USING (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles")))))) WITH CHECK (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles"))))));
...
CREATE POLICY "Admins can view integration logs" ON "public"."integration_logs" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."roles" && ARRAY['admin'::"text", 'super_admin'::"text"])))));



CREATE POLICY "Admins can view storage stats" ON "public"."storage_stats" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles")))))));
...
CREATE POLICY "Allow admins to view analytics events" ON "public"."analytics_events" FOR SELECT USING ("public"."is_admin_safe"());
...
CREATE POLICY "Analytics stats are managed by admins" ON "public"."analytics_stats" USING ("public"."is_admin_safe"()) WITH CHECK ("public"."is_admin_safe"());
...
CREATE POLICY "Analytics stats are readable by authenticated users" ON "public"."analytics_stats" FOR SELECT USING (true);
...
CREATE POLICY "Backups are only manageable by admins" ON "public"."backups" TO "authenticated" USING (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles")))))) WITH CHECK (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles"))))));
...
CREATE POLICY "New users can create their profiles" ON "public"."profiles" FOR INSERT WITH CHECK (("auth"."uid"() = "id"));



CREATE POLICY "Only admins can delete users" ON "public"."users" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ('admin'::"text" = ANY ("profiles"."roles"))))));



CREATE POLICY "Only admins can read audit logs" ON "public"."security_audit_logs" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND (("profiles"."roles" && ARRAY['admin'::"text", 'super_admin'::"text"]) OR ('super_admin'::"text" = ANY ("profiles"."roles")) OR ('admin'::"text" = ANY ("profiles"."roles")))))));



CREATE POLICY "Only admins can select sensitive data" ON "public"."users" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ('admin'::"text" = ANY ("profiles"."roles"))))));



CREATE POLICY "Only admins can view audit logs" ON "public"."security_audit_logs" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles")))))));



CREATE POLICY "Only admins can view events" ON "public"."analytics_events" FOR SELECT USING ("public"."is_admin_safe"());
...
CREATE POLICY "Settings are only editable by admins" ON "public"."settings" TO "authenticated" USING (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles")))))) WITH CHECK (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles"))))));
...
CREATE POLICY "Themes are only manageable by admins" ON "public"."themes" TO "authenticated" USING (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles")))))) WITH CHECK (("auth"."uid"() IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles"))))));
...
CREATE POLICY "Users can see their own profiles" ON "public"."profiles" FOR SELECT USING (("auth"."uid"() = "id"));



CREATE POLICY "Users can send messages to accessible channels" ON "public"."slack_messages" FOR INSERT WITH CHECK ((("auth"."uid"() IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM "public"."slack_channels"
  WHERE (("slack_channels"."id" = "slack_messages"."channel_id") AND (("slack_channels"."admin_only" = false) OR (EXISTS ( SELECT 1
           FROM "public"."profiles"
          WHERE (("profiles"."id" = "auth"."uid"()) AND (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles"))))))))))));
...
CREATE POLICY "Users can update their own profiles" ON "public"."profiles" FOR UPDATE USING (("auth"."uid"() = "id")) WITH CHECK (("auth"."uid"() = "id"));
...
CREATE POLICY "Users can view messages in accessible channels" ON "public"."slack_messages" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."slack_channels"
  WHERE (("slack_channels"."id" = "slack_messages"."channel_id") AND (("slack_channels"."admin_only" = false) OR (EXISTS ( SELECT 1
           FROM "public"."profiles"
          WHERE (("profiles"."id" = "auth"."uid"()) AND (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles")))))))))));
...
CREATE POLICY "Users can view non-admin channels and admins can view all" ON "public"."slack_channels" FOR SELECT USING ((("admin_only" = false) OR (EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND (('admin'::"text" = ANY ("profiles"."roles")) OR ('super_admin'::"text" = ANY ("profiles"."roles"))))))));
...
CREATE POLICY "allow_admin_insert_settings" ON "public"."settings" FOR INSERT WITH CHECK ("public"."is_admin_safe"());



CREATE POLICY "allow_admin_select_settings" ON "public"."settings" FOR SELECT USING ("public"."is_admin_safe"());



CREATE POLICY "allow_admin_update_settings" ON "public"."settings" FOR UPDATE USING ("public"."is_admin_safe"()) WITH CHECK ("public"."is_admin_safe"());
...
CREATE POLICY "profiles_insert" ON "public"."profiles" FOR INSERT WITH CHECK ((("auth"."uid"() = "id") OR ("auth"."uid"() IN ( SELECT "profiles_1"."id"
   FROM "public"."profiles" "profiles_1"
  WHERE ("profiles_1"."roles" && ARRAY['admin'::"text", 'super_admin'::"text"])))));



CREATE POLICY "profiles_select_admin" ON "public"."profiles" FOR SELECT USING ("public"."is_admin_safe"());
...
CREATE POLICY "profiles_update_admin" ON "public"."profiles" FOR UPDATE USING (("auth"."uid"() IN ( SELECT "profiles_1"."id"
   FROM "public"."profiles" "profiles_1"
  WHERE ("profiles_1"."roles" && ARRAY['admin'::"text", 'super_admin'::"text"]))));
...
GRANT ALL ON FUNCTION "public"."get_all_profiles"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_all_profiles"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_all_profiles"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_analytics_data"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_analytics_data"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_analytics_data"() TO "service_role";
